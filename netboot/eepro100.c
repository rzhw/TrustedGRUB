...
static void eepro100_reset(struct nic *card)
{
...
static void eepro100_transmit(struct nic *card, const char *d, unsigned int t, unsigned int s, const char *p)
{
  struct eth_hdr {
  ...
  memcpy (&hdr.src_addr, card->node_addr, ETH_ALEN);
 ...
 * side effects:
 *            returns the packet in the array card->packet.
 *            returns the length of the packet in card->packetlen.
 */

static int eepro100_poll(struct nic *card)
{
...
#endif
  card->packetlen =  ACCESS(rxfd)count & 0x3fff;
  memcpy (card->packet, ACCESS(rxfd)packet, card->packetlen);
#ifdef	DEBUG
  hd (card->packet, 0x30);
#endif
  return 1;
...
static void eepro100_disable(struct nic *card)
{
...
struct nic *eepro100_probe(struct nic *card, unsigned short *probeaddrs, struct pci_device *p)
{
	...
	for (i = 0, sum = 0; i < ee_size; i++) {
		value = do_eeprom_cmd(read_cmd | (i << 16), 27);
  ...
  for (i=0;i<ETH_ALEN;i++) {
	card->node_addr[i] =  (eeprom[i/2] >> (8*(i&1))) & 0xff;
  }
  ...
  ACCESS(rxfd)rx_buf_addr = (int) &card->packet;
  ACCESS(rxfd)count   = 0;
  ...
  card->reset = eepro100_reset;
  card->poll = eepro100_poll;
  card->transmit = eepro100_transmit;
  card->disable = eepro100_disable;
  return card;
}
