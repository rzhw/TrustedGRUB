 ...
 ***/
static void
a3c90x_transmit(struct nic *card, const char *d, unsigned int t,
 ...
 *** copy the packet to card->packet if it gets a packet and set the size
 *** in card->packetlen.  Return 1 if a packet was found.
 ***/
static int
a3c90x_poll(struct nic *card)
    {
    int i, errcode;
    ...
    INF_3C90X.ReceiveUPD.DataAddr = virt_to_bus(card->packet);
    ...
    /** Ok, got packet.  Set length in card->packetlen. **/
    card->packetlen = (INF_3C90X.ReceiveUPD.UpPktStatus & 0x1FFF);
 ...
 ***/
static void
a3c90x_disable(struct nic *card)
    {
 ...
 ***/
struct nic*
a3c90x_probe(struct nic *card, unsigned short *probeaddrs, struct pci_device *pci)
    {
    int i, c;
	...
	card->node_addr[i] = (eeprom[i/2] >> (8*((i&1)^1))) & 0xff;
    ...
    card->reset    = a3c90x_reset;
    card->poll     = a3c90x_poll;
    card->transmit = a3c90x_transmit;
    card->disable  = a3c90x_disable;
