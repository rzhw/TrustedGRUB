...
/* BEGIN TCG EXTENSION */
// Here all commands from menu.lst and the console are measured and
// extended into PCR 12
void extend_cmdline_into_pcr(unsigned char* grub_cmdline)
{
    int i;
    unsigned long hash_result[5];
    sha1_context cmdline_sha1;

    sha1_init(&cmdline_sha1);
    sha1_update(&cmdline_sha1,grub_cmdline,strlen(grub_cmdline));
    sha1_finish(&cmdline_sha1,hash_result);

#ifdef SHOW_SHA1
    if ((xy++)>17){ cls(); xy=0; }
    gotoxy(0,(xy&0xff));
    printf("Cmdl:[                                        ]->PCR[%d] ",PCR_CMDLINE);

    for (i=0; i<22; i++)
    {
	if (grub_cmdline[i])
	    printf("%c",grub_cmdline[i]);
	else
	    break;
    }
    if (i<22)
    ...
    if (!(tpm_present()))
        update_pcr(PCR_CMDLINE,hash_result);
}			    
/* END TCG EXTENSION */
