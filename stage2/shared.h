...
/* Constants and memory locations which are needed to make the TCG extension work properly. */

/* segment for storing data required for TCG functions */
#define TCG_SEG          0x8000
/* address corresponding with the segment defined above */
#define TCG_BUFFER_ADDR  RAW_ADDR(0x80000)
/* maximal size of data that can be computed in one single step */
#define TCG_BUFFER_SIZE  4096

#define PCR_CMDLINE	12
#define PCR_CHECKFILE	13
#define PCR_KERNEL 	14
...
extern int old_decompression_value;
...
/* Loads checkfile to measure integrity of arbitrary files.
The actual function is defined in the file boot.c.*/
int load_checkfile (char *checkfile);
...
/* SHA1 calculation. The actual functions are defined in the file stage2/sha1.c.*/
extern int sha1_init(sha1_context *ctx );
extern int sha1_update(sha1_context *ctx, t_U8 *chunk_data, t_U32 chunk_length);
extern int sha1_finish(sha1_context *ctx, t_U32 *sha1_hash);
extern int calculate_sha1 (char *filename, unsigned long *hash_result, int print_results);

// global SHA1-context
sha1_context my_sha1;

// Extern variables needed for SHA1
extern int perform_sha1;
extern int sha1_byte_count;
extern int measure_kernel;
extern int old_perform_sha1_value;
extern int update_pcr(unsigned char pcr, unsigned long *hash_result);
extern int xy;

/* Prototype for additional measurement functions defined in file asm.S.
   For details see README file. */

// Shall we look for a TPM?
long check_for_tpm (void);
// Checks for a TCG BIOS
long tcg_check_tpm (void);
// tpm_present() returns a 0 if we have a TPM and 0xbb00 if we don't
long tpm_present (void);
//give_tpm_answer() returns the answer from the TCG BIOS
long give_tpm_answer (void);

// calls TCG_Extend_PCR. The parameters are given in the function "update_pcr" in stage2/boot.c
