...
/* BEGIN TCG EXTENSION */
// Disable SHA1-measurement for loadage of the new configfile
if (perform_sha1)
{
    old_perform_sha1_value = perform_sha1;
    perform_sha1 = 0;
...
/* The new checkfile command. The actual code is located in the file boot.c, this one here 
is only a backend for proper integration into GRUB and error management.*/

/* checkfile */

static int
checkfile_func (char *arg, int flags)
{
  int result;

  /* Tries to compute the checkfile given as argument. */
  result = load_checkfile(arg);

  /* If an error occurs, booting is stopped. */
  if (result)
    {
...
/* sha1 */

static int sha1_func (char *arg, int flags)
{
    unsigned long hash_result[5];
    if (calculate_sha1(arg, hash_result,1))
    {
...
/* Integrates the sha1 mechanism into GRUB's command structure. */

static struct builtin builtin_sha1 =
{
  "sha1",
  sha1_func,
  BUILTIN_CMDLINE,
  "Calculate SHA1",
  "Calcualtes SHA1 of the given file."
};

/* Integrates the checkfile mechanism into GRUB's command structure. */

static struct builtin builtin_checkfile =
{
  "checkfile",
  checkfile_func,
  BUILTIN_CMDLINE,
  "Load checkfile",
  "Load checkfile containing list of files that have to be measured."
};
...
int boot_serial_unit = 0;
unsigned int boot_serial_speed = 0;
...
struct builtin *builtin_table[] =
{
  ...
  &builtin_cat,
  &builtin_chainloader,
  &builtin_checkfile,    /* newly added for TCG functionality */
  &builtin_cmp,
  &builtin_color,
  &builtin_configfile,
  &builtin_debug,
  ...
  &builtin_setkey,
  &builtin_setup,
  &builtin_sha1,    	 /* newly added for TCG functionality */
#if defined(SUPPORT_SERIAL) || defined(SUPPORT_HERCULES)
  &builtin_terminal,
#endif /* SUPPORT_SERIAL || SUPPORT_HERCULES */
#ifdef SUPPORT_SERIAL
